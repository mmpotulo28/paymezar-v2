...
export function PricingCard({ plan, isYearly, isCurrent = false }: PricingCardProps) {
	...
	return (
		<div className="relative cursor-pointer" onClick={handleClick}>
			{!isCurrent && plan.isPopular && (
				<div
					className="absolute left-1/2 -translate-x-1/2 -top-3 px-4 py-1.5
                      text-xs font-medium bg-primary text-white rounded-full z-10 flex flex-row items-centre gap-2">
					<StarIcon size={12} /> Most Popular
				</div>
			)}
			...
			{isCurrent && (
				<div
					className="absolute left-1/2 -translate-x-1/2 -top-3 px-4 py-1.5
                      text-xs font-medium bg-primary text-white rounded-full z-10 flex flex-row items-center gap-2">
					<CheckIcon size={12} /> Current Plan
				</div>
			)}

			<Card
				className={`w-full ${plan.isPopular || isCurrent ? "border-2 border-primary" : ""}`}
				shadow="md">
