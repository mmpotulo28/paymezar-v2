    ...
    if (data) {
      try {
        ...
        const isValidPayMezarRequest = () =>
          isPayMezarRequest && hasRecipient && hasAmount;
        ...
        if (isValidPayMezarRequest()) {
          ...
          setError(
            "QR code does not contain a valid PayMe-Zar payment request.",
          );
        }
      } catch (e) {
    ...
    <Card className="w-full max-w-2xl">
      ...
      <CardFooter className="flex items-center justify-end">
        <span className="text-xs text-default-500 flex items-center gap-1">
          <AlertCircleIcon className="text-danger" size={16} /> Be cautious when
          sending payments. Always verify recipient details.
        </span>
      </CardFooter>
      {scanModalOpen && (
        <QrCodeScanner
